...
describe("InterleavedData", () => {
  ...
  describe("count", () => {
    it("returns array.length / stride", () => {
      const buf = new InterleavedData(new Float32Array(12), 3);
      expect(buf.count).toBe(4);
    });

    it("stride=2", () => {
      const buf = new InterleavedData(new Float32Array(8), 2);
      expect(buf.count).toBe(4);
    });
  });
  ...
  describe("set", () => {
    ...
    it("returns this", () => {
      const buf = new InterleavedData(new Float32Array(3), 3);
  ...
  describe("clone", () => {
    it("returns independent copy with same data", () => {
      ...
      const buf = new InterleavedData(arr, 2);
