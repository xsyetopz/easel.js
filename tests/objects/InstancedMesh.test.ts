...
describe("InstancedMesh", () => {
  it("has type='InstancedMesh'", () => {
    expect(new InstancedMesh(undefined, undefined, 1).type).toBe(
      "InstancedMesh",
    );
  });

  it("count getter returns constructor count", () => {
    const mesh = new InstancedMesh(undefined, undefined, 5);
    expect(mesh.count).toBe(5);
  });

  it("instanceMatrix is a Float32Array of length count*16", () => {
    const mesh = new InstancedMesh(undefined, undefined, 3);
