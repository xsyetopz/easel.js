...
function buildSphereData(opts: SphereBuildOptions): SphereBuildData {
  const ws = Math.max(3, Math.floor(opts.widthSegments));
  const hs = Math.max(2, Math.floor(opts.heightSegments));
  ...
  for (let iy = 0; iy <= hs; iy++) {
    ...
    for (let ix = 0; ix <= ws; ix++) {
      const u = ix / ws;
      const phi = opts.phiStart + u * opts.phiLength;
      const theta = opts.thetaStart + v * opts.thetaLength;
      ...
      positions.push(opts.radius * nx, opts.radius * ny, opts.radius * nz);
  ...
  for (let iy = 0; iy < hs; iy++) {
    for (let ix = 0; ix < ws; ix++) {
      ...
      if (iy !== 0 || opts.thetaStart > 0) indices.push(a, b, d);
      if (iy !== hs - 1 || opts.thetaLength + opts.thetaStart < Math.PI) {
        indices.push(b, c, d);
      }
    }
  }

  return { positions, normals, uvs, indices, vertexCount: index };
}
