import { Geometry } from "../Geometry.ts";

interface LathePoint {
  x: number;
  y: number;
}

interface PhiParams {
  seg: number;
  phiStart: number;
  inverseSegments: number;
  phiLength: number;
}

interface LatheState {
  positions: number[];
  normals: number[];
  uvs: number[];
  indices: number[];
  index: number;
  seg: number;
  phiStart: number;
  inverseSegments: number;
  phiLength: number;
  points: LathePoint[];
}

interface LatheGridResult {
  positions: number[];
  uvs: number[];
  grid: number[][];
  index: number;
}

function buildCap(state: LatheState, pointIndex: number, sign: number): number {
  const {
    positions,
    normals,
    uvs,
    indices,
    seg,
    phiStart,
    inverseSegments,
    phiLength,
    points,
  } = state;
  let { index } = state;

  const centerY = points[pointIndex].y;
  const centerIdx = index;
  positions.push(0, centerY, 0);
  normals.push(0, sign, 0);
  uvs.push(0.5, 0.5);
  index++;

  for (let s = 0; s <= seg; s++) {
    const phi = phiStart + s * inverseSegments * phiLength;
    const cos = Math.cos(phi);
    const sin = Math.sin(phi);
    const r = points[pointIndex].x;
    positions.push(r * sin, centerY, r * cos);
    normals.push(0, sign, 0);
    uvs.push(sin * 0.5 * sign + 0.5, cos * 0.5 + 0.5);
    index++;
  }

  for (let s = 0; s < seg; s++) {
    const first = centerIdx + s + 1;
    const second = centerIdx + s + 2;
    if (sign > 0) {
      indices.push(centerIdx, first, second);
    } else {
      indices.push(second, first, centerIdx);
    }
  }

  return index;
}

function computeNormals(
  points: LathePoint[],
  normals: number[],
  opts: PhiParams,
): void {
  for (let i = 0; i < points.length; i++) {
    const ip = Math.min(i + 1, points.length - 1);
    const im = Math.max(i - 1, 0);
    const tx = points[ip].x - points[im].x;
    const ty = points[ip].y - points[im].y;
    for (let s = 0; s <= opts.seg; s++) {
      const phi = opts.phiStart + s * opts.inverseSegments * opts.phiLength;
      const sin = Math.sin(phi);
      const cos = Math.cos(phi);
      const nx = ty * cos;
      const ny = -tx;
      const nz = ty * sin;
      const len = Math.sqrt(nx * nx + ny * ny + nz * nz);
      if (len > 0) {
        normals.push(nx / len, ny / len, nz / len);
      } else {
        normals.push(0, 1, 0);
      }
    }
  }
}

function buildIndices(
  grid: number[][],
  indices: number[],
  pointCount: number,
  seg: number,
): void {
  for (let i = 0; i < pointCount - 1; i++) {
    for (let s = 0; s < seg; s++) {
      const a = grid[i][s];
      const b = grid[i + 1][s];
      const c = grid[i + 1][s + 1];
      const d = grid[i][s + 1];
      indices.push(a, d, b);
      indices.push(b, d, c);
    }
  }
}

function buildLatheGrid(
  points: LathePoint[],
  opts: PhiParams,
): LatheGridResult {
  const positions: number[] = [];
  const uvs: number[] = [];
  const grid: number[][] = [];
  const inversePointCount = 1 / (points.length - 1);
  let index = 0;

  for (let i = 0; i < points.length; i++) {
    const row: number[] = [];
    const t = i * inversePointCount;
    for (let s = 0; s <= opts.seg; s++) {
      const phi = opts.phiStart + s * opts.inverseSegments * opts.phiLength;
      const sin = Math.sin(phi);
      const cos = Math.cos(phi);
      positions.push(points[i].x * sin, points[i].y, points[i].x * cos);
      uvs.push(s * opts.inverseSegments, t);
      row.push(index++);
    }
    grid.push(row);
  }

  return { positions, uvs, grid, index };
}

function buildCapsIfNeeded(state: LatheState, points: LathePoint[]): number {
  let index = state.index;
  if (points[0].x > 1e-6) {
    index = buildCap(state, 0, -1);
    state.index = index;
  }
  const last = points.at(-1);
  if (last && last.x > 1e-6) {
    index = buildCap(state, points.length - 1, 1);
  }
  return index;
}

/** Geometry generated by revolving a 2D profile around the Y axis. */
export class LatheGeometry extends Geometry {
  /** Serialization discriminator for this runtime type. */
  declare type: string;
  /** Primitive-construction parameters retained for serialization. */
  declare parameters: Record<string, unknown>;

  /** Revolves a 2D profile around +Y with the requested radial segmentation. */
  constructor(
    points: LathePoint[],
    segments: number = 12,
    phiStart: number = 0,
    phiLength: number = Math.PI * 2,
  ) {
    super();

    this.type = "LatheGeometry";
    this.parameters = { points, segments, phiStart, phiLength };

    const seg = Math.floor(segments);
    const inverseSegments = 1 / seg;
    const phiParams: PhiParams = {
      seg,
      phiStart,
      inverseSegments,
      phiLength,
    };

    const {
      positions,
      uvs,
      grid,
      index: vertexIndex,
    } = buildLatheGrid(points, phiParams);
    const normals: number[] = [];
    const indices: number[] = [];

    computeNormals(points, normals, phiParams);
    buildIndices(grid, indices, points.length, seg);

    let index = vertexIndex;
    const fullRevolution = phiLength >= Math.PI * 2 - 1e-6;

    if (fullRevolution) {
      const state: LatheState = {
        positions,
        normals,
        uvs,
        indices,
        index,
        seg,
        phiStart,
        inverseSegments,
        phiLength,
        points,
      };
      index = buildCapsIfNeeded(state, points);
    }

    const IndexArray = index > 65535 ? Uint32Array : Uint16Array;

    this.setPositions(new Float32Array(positions));
    this.setNormals(new Float32Array(normals));
    this.setUVs(new Float32Array(uvs));
    this.index = new IndexArray(indices);
  }
}
