...
export class LightBaker {
	 ...
	 * @param {{ triangles: Array<{ normal: { x: number, y: number, z: number }, vertices: Array<{ normal: { x: number, y: number, z: number } }> }>, material: { flatShading?: boolean }, shadedColors: Array<*> }} drawCall
	...
	bake(drawCall, lights) {
		...
		for (let i = 0; i < drawCall.triangles.length; i++) {
			...
			if (drawCall.material.flatShading) {
