...
/** Untrusted JSON object accepted by ObjectLoader parsing helpers. */
export type ObjectRecord = Record<string, unknown> & {
  animations?: unknown;
  background?: unknown;
  boneInverses?: unknown;
  bones?: unknown;
  center?: unknown;
  children?: unknown;
  elements?: unknown;
...
/** Tests whether an unknown value is a non-array object record. */
...
/** Reads a finite numeric field, returning a fallback when it is absent. */
...
/** Reads a required finite numeric field from an ObjectLoader record. */
...
/** Reads a boolean field, using a fallback only when the field is absent. */
...
/** Reads a string field, using a fallback only when the field is absent. */
...
/** Validates and returns a finite numeric tuple when the field is present. */
...
/** Reads an optional nested record and rejects arrays or null values. */
...
/** Restores a serialized UUID as a non-writable enumerable object property. */
export function hydrateUuid(
  object: { uuid: string },
  json: ObjectRecord,
): void {
...
/** Converts the serialized 27-component array into spherical harmonics. */
export function parseSphericalHarmonics(
  json: ObjectRecord,
): SphericalHarmonics3 {
...
/** Applies serialized background and fog settings to a scene. */
...
/** Applies a serialized view offset to a perspective or orthographic camera. */
