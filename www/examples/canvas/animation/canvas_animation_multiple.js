...
export const meta = {
  ...
  category: "canvas",
  description:
    "Two CPU clips blend across several authored roots in one animation group.",
};
...
const group = new THREE.AnimationObjectGroup(meshA, meshB, meshC, meshD);
const spin = new THREE.AnimationClip("spin", 2.8, [
  new THREE.NumberKeyframeTrack(".rotation[y]", [0, 1.4, 2.8], [0, Math.PI, Math.PI * 2]),
]);
const bounce = new THREE.AnimationClip("bounce", 1.4, [
  new THREE.NumberKeyframeTrack(".position[y]", [0, 0.7, 1.4], [0.7, 1.55, 0.7]),
]);
const mixer = new THREE.AnimationMixer(group);
mixer.clipAction(spin).setLoop(THREE.LoopRepeat, Number.POSITIVE_INFINITY).play();
mixer.clipAction(bounce).setLoop(THREE.LoopRepeat, Number.POSITIVE_INFINITY).play();
mixer.update(delta);`;
